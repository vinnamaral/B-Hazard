...
uses
  ...
  Dialogs, jpeg, ExtCtrls, Buttons, StdCtrls, Menus, ExtDlgs, ImgList, XPMan;
...
begin
  FormCd:= TFormCd.Create(self);
  FormCd.ShowModal;
end;

procedure TFormPrincipal.Dvds1Click(Sender: TObject);
begin
  FormDvd:= TFormDvd.Create(self);
  FormDvd.ShowModal;
end;
...
begin
  FormJogos:= TFormJogos.Create(self);
  FormJogos.ShowModal;
end;

procedure TFormPrincipal.SpeedButton5Click(Sender: TObject);
begin
  FormLivros:= TFormLivros.Create(self);
